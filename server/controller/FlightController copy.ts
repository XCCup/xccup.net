...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { igcContent, igcName } = req.body;

    console.log("##########################");
    console.log("Name:", igcName);

    const igcFilePath = await storeIgcFile(igcContent, igcName);
    ...
    //   // G-Check
    //   const { content, encoding } = readIgcFile(igcFile.path);

    //   // logger.error("IFC FILE: " + JSON.stringify(igcFile, null, 2));

    //   const filename = igcFile.filename;
    const validationResult = await validateIgc(igcContent, igcName);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // const { content, encoding } = readIgcFile(igcFile.path);
      const filename = igcFile.filename;
      const validationResult = await validateIgc(igcFile.path, filename);
...
async function storeIgcFile(igcContent: string, igcName: string) {
  const dataPath = config.get("dataPath");

  const externalId = await FlightService.createExternalId();
  const filename = externalId + "_" + igcName;

  const destination = path
    .join(dataPath, IGC_STORE, getCurrentYear().toString(), filename)
    .toString();

  fs.writeFileSync(destination, Buffer.from(igcContent, "base64"));
