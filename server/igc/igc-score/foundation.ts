...
import * as vincentys from "./vincentys";
export class Point {
  constructor(x, y) {
    if (Array.isArray(x)) {
  ...
  intersects(other) {
    ...
    if (other instanceof Box) return other.intersects(this);
  ...
  distanceEarthRev(dx, dy) {
    const lon =
      this.x + util.degrees(dx / util.REarth / Math.cos(util.radians(this.y)));
...
export class Range {
  constructor(start, end) {
    this.start = start;
    this.end = end;
    if (end < start) throw new Error("start should be before end");
  }
  ...
  right() {
    return new Range(
      this.start + Math.ceil((this.end - this.start) / 2),
      this.end
    );
  }
