...
const tableDir = path.resolve(__dirname, "../oldresults/seniors");
...
files.forEach((file) => {
  ...
  results.push({ season, type: "seniors", result: converted });
});

fs.writeFile(
  "oldSeniorRankings.json",
...
function findTeamId(teamName, season) {
  ...
  console.log(`Found no club for ${teamName}`);
}
